...
const getConfig = (type: QuizDataType): { system: string, schema: Schema } => {
    switch (type) {
        ...
        case 'fishing':
            return {
                system: `把prompt出成英语高考小猫钓鱼题（十一选十），高考难度。\n在text里输出挖空的文本（**只需**将挖空词以<code></code>包裹，形如<code>word</code>），在distrators输出干扰项数组。必须对下文出十空，必须只对其中一空出一个干扰项（易混淆但有明显区别），例如{"text":"...<code>word</code>...","distractors":["lexicon"]}}。不要输出任何序号。\n一共出十题，即你必须**对十个单词进行挖空**，并另外添加**一个干扰项**（要求与原文对应词词类相同、易混淆但词义差异显著）。禁止对同一单词反复挖空，尽可能广泛地考察词性不同的词汇，选项中尽量使用常用词。挖空词汇必须与上下文强关联，可以合理推断出答案。挖空分布必须均匀分布，所以要事先做好规划，确保贯穿全文每一段落。必须保留其余原有的HTML格式、换行（用换行作为分段）、标签，不要在挖空前加编号。`,
                system: `把prompt出成英语高考小猫钓鱼题（十一选十），高考难度。\n在text里输出挖空的文本（**只需**将挖空词以<code></code>包裹，形如<code>word</code>），在distrators输出干扰项数组。必须对下文出十空，并只对其中一空出一个干扰项（易混淆但有明显区别），例如{"text":"...<code>word</code>...","distractors":["lexicon"]}}。不要输出任何序号。\n一共出十题，即你必须**对十个单词进行挖空**，并另外添加**一个干扰项**（要求与原文对应词词类相同、易混淆但词义差异显著）。禁止对同一单词反复挖空，尽可能广泛地考察词性不同的词汇，选项中尽量使用常用词。挖空词汇必须与上下文强关联，可以合理推断出答案。挖空分布必须均匀分布，所以要事先做好规划，确保贯穿全文每一段落。必须保留其余原有的HTML格式、换行（用换行作为分段）、标签，不要在挖空前加编号。`,
                schema: z.object({
        ...
        case 'reading':
            return {
                system: `把prompt出成英语高考阅读理解题，高考难度。\n在questions输出题目数组。每个题目以q为问题，a为四个选项，correct为正确选项下标。\n一共出五题，考察内容均匀。供参考的出题方向：事实信息题、内容推断题（What can you infer）、标题推测题（What is the best title，仅当标题未给定时出）、句子作用题、生僻单词/生僻词组语义推测题、态度判断题（What is the author's attitude）……必须保留其余原有的HTML格式、换行（用换行作为分段）、标签，不要在挖空前加编号。`,
                schema: z.object({
                    questions: z.array(z.object({
