...
export function SubjectiveInput({
    ...
    return (
        <Textarea
            value={currentAnswer}
            onValueChange={value =>
                setAnswer({ sectionId: groupId, localQuestionNo: localNo, option: value })
            }
...
function SummaryReviseFeedback({
    answer,
    feedback,
}: {
    answer: string
        ...
        <div className='mt-4 flex flex-col gap-3'>
            <div className='flex items-baseline'>
                ...
                <span className='text-sm text-default-500 ml-2 space-x-3'>
                    <span>
                        内容 <span className='font-mono'>{feedback.contentScore}/5</span>
                    </span>
                    <span>
                        语言 <span className='font-mono'>{feedback.languageScore}/5</span>
                    </span>
                </span>
            </div>
                        ...
                        {feedback.essentialItemResults.map((r, i) => (
                            <li key={i} className='flex items-start gap-2 text-sm pl-0'>
                                ...
                                <span>
                                    {r.item}
                                    {r.note && (
                                        <span className='text-default-400'> — {r.note}</span>
                                    )}
                                </span>
                            </li>
                        ))}
            ...
            {feedback.rationale && (
                <p className='text-sm text-default-600 leading-relaxed'>
                    {fixDumbPunctuation(feedback.rationale)}
                </p>
            )}
        </div>
    )
        ...
        <span>
            {parts.map((part, i) => {
                ...
                return isHighlighted ? (
                    <mark
                        key={i}
                        className='bg-warning-100 dark:bg-warning-100 text-inherit rounded px-0.5'
                    >
                        {part}
                    </mark>
                ) : (
...
function TranslationItemReviseFeedback({
    answer,
    itemFeedback,
    groupId,
    localNo,
                ...
                <div className='flex items-baseline gap-2 flex-1'>
                    <span className='text-lg font-semibold font-mono'>
                        ...
                        <span className='text-default-400 text-sm font-mono'>
                            /{itemFeedback.maxScore}
                        </span>
                    </span>
                    {itemFeedback.rationale && (
                        <span className='text-sm text-default-600 leading-relaxed flex-1'>
                            {fixDumbPunctuation(itemFeedback.rationale)}
                        </span>
                    )}
                </div>
                <SaveQuestionNoteButton
                    isIconOnly
                    size='sm'
                    variant='light'
                    localNo={localNo}
...
function buildAnnotations(
    ...
    for (const pair of badPairs) {
        ...
        if (idx !== -1) {
            annotations.push({
                start: idx,
                end: idx + pair.original.length,
                kind: 'bad',
                detail: pair.improved,
            })
        }
    }
                    ...
                    {annotation.kind === 'bad' ? (
                        ...
                        <>
                            ...
                            <Streamdown className='text-sm'>
                                {fixDumbPunctuation(annotation.detail)}
                            </Streamdown>
                        </>
                    )}
                </div>
...
function WritingReviseFeedback({
    answer,
    feedback,
}: {
    answer: string
    ...
    return (
        <div className='mt-6 flex flex-col gap-4'>
            <div className='flex items-baseline'>
                ...
                <span className='text-sm text-default-500 ml-2 space-x-3'>
                    <span>
                        内容 <span className='font-mono'>{feedback.contentScore}/10</span>
                    </span>
                    <span>
                        语言 <span className='font-mono'>{feedback.languageScore}/10</span>
                    </span>
                    <span>
                        结构 <span className='font-mono'>{feedback.structureScore}/5</span>
                    </span>
                </span>
            </div>
            ...
            {feedback.corrected && (
                <div className='flex flex-col'>
                    <div
                        className='prose dark:prose-invert max-w-none font-mono -mt-2'
                        dangerouslySetInnerHTML={{ __html: mdToHtml(feedback.corrected) }}
                    />
                </div>
            )}
        </div>
    ...
    return (
        <div className='mt-3 flex flex-col gap-1'>
            ...
            <div className='flex justify-end pr-1'>
                <span
                    className={`text-xs font-mono ${wordCount >= 60 ? 'text-danger font-medium' : wordCount >= 50 ? 'text-warning font-medium' : 'text-default-400'}`}
                >
                    {wordCount} / 60
                </span>
            </div>
...
function buildAppealContext(
    ...
    if (section.type === 'writing') {
        ...
        return [`## 写作要求`, data.guidance, ``, `## 考生作答`, answers[1] || '（未作答）'].join(
            '\n',
        )
    }
        ...
        return (
            <div className='mt-3 mb-8'>
                <Button
                    ...
                    isLoading={isPending}
                    onPress={() => {
                        if (paperId) execute({ paperId: parseInt(paperId, 10) })
                    }}
                >
                    检查批改进度
                </Button>
 ...
 */
export function SubjectiveSectionTitle({
    groupId,
    children,
}: {
    groupId: string
