                    ...
                    {() => (
                        <>
                            ...
                            <ModalBody className='px-6 py-2'>
                                <div
                                    ref={scrollRef}
                                    className='flex flex-col gap-3 min-h-48 max-h-96 overflow-y-auto py-3'
                                >
                                    {messages.length === 0 ? (
                                        ...
                                        messages.map(msg => (
                                            <div
                                                key={msg.id}
                                                className={`text-sm leading-relaxed px-4 py-3 rounded-large max-w-[85%] whitespace-pre-wrap ${
                                                    msg.role === 'user'
                                            ...
                                            >
                                                {msg.parts.map((part, i) =>
                                                    'text' in part ? (
                                                        <Streamdown
                                                            key={`${msg.id}-${i}`}
                                                            className='prose-headings:my-3 prose-p:not-first:mt-2'
                                                        >
                                                            {part.text}
                                                        </Streamdown>
                                                    ) : null,
