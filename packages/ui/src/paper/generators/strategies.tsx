        ...
        <section>
            {data.questions.map((q, index) => (
                <div key={index} className='flex gap-x-2 listening-item'>
                    ...
                    <table className='my-0 not-prose'>
                        <tbody>
                            {toTableRows(
                                (q.a || []).map((option, i) => (
                                    <td key={i}>
                                        {ALPHABET_ELEMENTS[i]} {option}
                                    </td>
                                )),
                                1,
                            )}
...
const grammarStrategy: QuestionStrategy<GrammarData> = createQuestionStrategy<GrammarData>({
    ...
    renderPaper: ({ data, config }) => {
        const parsedContent = replaceBlanks(
            data.text,
            config.start ?? 1,
            (displayNo, localNo, originalContent) => {
                const hint = data.hints[originalContent]
                const blank = (
                    <FillInTheBlank
                        blankCount={hint ? 1 : originalContent.split('/')[0].split(' ').length}
                        displayNo={displayNo}
                        localNo={localNo}
                        groupId={data.id}
                    />
...
const fishingStrategy: QuestionStrategy<FishingData, string[]> = createQuestionStrategy<
    FishingData,
    string[]
>({
    keyPerLine: 5,
    ...
    getLlmReadyText: data => {
        const textWithBlanks = data.text
            .replace(/<code>(.*?)<\/code>/g, '_____')
            .replace(/<[^>]*>?/gm, '')
        const correct = extractCodeContent(data.text)
        const allOptions = [...correct, ...data.distractors]
        const options = fastShuffle(getSeed(allOptions.join('')) * 1000, allOptions)
        const optionsWithLetters = options
            .map((opt, idx) => `${ALPHABET_SET[idx]} ${opt}`)
            .join(', ')
                ...
                <section className='my-2'>
                    <table className='print:break-inside-avoid border border-default-900 flex flex-wrap px-2 py-1'>
                        <tbody>
                            {toTableRows(
                                options.map((opt, idx) => (
                                    <td key={opt} className='whitespace-nowrap first:ml-2 py-0.5'>
                                        {ALPHABET_ELEMENTS[idx]} {opt}
                                    </td>
                                )),
                                11,
                            )}
...
const clozeStrategy: QuestionStrategy<ClozeData, Record<string, string[]>> = createQuestionStrategy<
    ClozeData,
...
>({
    ...
    getLlmReadyText: data => {
        const textWithBlanks = data.text
            .replace(/<code>(.*?)<\/code>/g, '_____')
            .replace(/<[^>]*>?/gm, '')
        const optionsText = data.questions
            .map((q, index) => {
                const choices = [q.original, ...(q.distractors ?? [])]
                const shuffled = fastShuffle(getSeed(choices.join('')) * 1000, choices)
                const withLetters = shuffled
                    .map((choice, i) => `${ALPHABET_SET[i]} ${choice}`)
                    .join(', ')
        ...
        const Options = () => (
            <section className='my-2 flex flex-col gap-y-2 print:gap-y-0'>
                {data.questions.map((q, index) => {
                    ...
                    if (!data.text.includes(`<code>${q.original}</code>`)) return null
                    return (
                                ...
                                <tbody>
                                    {toTableRows(
                                        options.map((option, i) => (
                                            <td key={i} className='text-left flex-1/5 text-nowrap'>
                                                {ALPHABET_ELEMENTS[i]} {option}
                                            </td>
                                        )),
                                        4,
        ...
        return (
            <>
                ...
                <Accordion
                    ...
                    itemProps={{
                        title: '完形填空选项',
                        className: 'print:hidden -ml-2 -mr-2 -mb-2',
                        classNames: { content: 'pt-0', title: 'font-medium' },
                    }}
                >
                    <Options />
                </Accordion>
    ...
    getDefaultValue: () => ({
        ...
        questions: [
            {
                original: 'purpose',
                distractors: ['history', 'location', 'title'],
            },
            {
                original: 'professionally',
                distractors: ['secretly', 'honestly', 'roughly'],
            },
            {
                original: 'prefer',
                distractors: ['cancel', 'seek', 'decline'],
            },
            {
                original: 'freely',
                distractors: ['wisely', 'loudly', 'carefully'],
            },
            {
                original: 'however',
                distractors: ['otherwise', 'besides', 'moreover'],
            },
            {
                original: 'selfishness',
                distractors: ['creativity', 'confidence', 'curiosity'],
            },
            {
                original: 'convenience',
                distractors: ['comfort', 'security', 'poverty'],
            },
            {
                original: 'closer',
                distractors: ['similar', 'kinder', 'loyal'],
            },
            {
                original: 'tone',
                distractors: ['speed', 'volume', 'topic'],
            },
            {
                original: 'casual',
                distractors: ['serious', 'tempting', 'relevant'],
            },
            {
                original: 'though',
                distractors: ['unless', 'until', 'since'],
            },
            {
                original: 'one-sided',
                distractors: ['strong-worded', 'ill-tempered', 'off-limits'],
            },
            {
                original: 'missed',
                distractors: ['regretted', 'requested', 'ignored'],
            },
            {
                original: 'hijacked',
                distractors: ['deleted', 'robbed', 'distracted'],
            },
            {
                original: 'nonsense',
                distractors: ['memory', 'closeness', 'efficiency'],
            },
        ],
    }),
})
...
const readingStrategy: QuestionStrategy<ReadingData> = createQuestionStrategy<ReadingData>({
    ...
    getLlmReadyText: data => {
        ...
        const questionsText = data.questions
            .map((q, index) => {
                const optionsWithLetters = q.a
                    .map((opt, i) => `${ALPHABET_SET[i]} ${opt}`)
                    .join('\n')
                return `${index + 1}. ${q.q}\n${optionsWithLetters}`
            })
            .join('\n\n')
                ...
                {data.questions.map((q, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            id={`q${displayNo}`}
                            className='flex flex-col gap-1 prose-code:before:hidden prose-code:after:hidden prose-code:italic prose-code:font-ui prose-code:text-medium prose-code:font-medium'
                        >
                            <div>
                    ...
                    <div key={index} className='flex gap-2 flex-col'>
                        <p>
                            <span
                                className={cn(
                                    'font-bold',
                                    !answers[data.id]?.[localNo] && 'text-secondary',
                                )}
                            >
                                {displayNo}.{' '}
                            </span>
...
const sentenceChoiceStrategy: QuestionStrategy<SentenceChoiceData, string[]> =
    createQuestionStrategy<SentenceChoiceData, string[]>({
        scorePerQuestion: 2,
        keyPerLine: 4,
        renderRubric: () => (
            <>
                    ...
                    <section className='my-2'>
                        <table className='border border-default-900 print:break-inside-avoid '>
                            <tbody>
                                {toTableRows(
                                    options.map((opt, idx) => (
                                        <td key={opt} className='mx-3 py-1'>
                                            {ALPHABET_ELEMENTS[idx]} {opt}
                                        </td>
                                    )),
                                    1,
...
const customStrategy: QuestionStrategy<CustomData> = createQuestionStrategy<CustomData>({
    ...
    getQuestionCount: ({ key }) => {
        if (key.replace(/<[^>]*>/g, '').length > 0) return 1
        else return 0
    },
    getCorrectAnswers: () => [],
...
const translationStrategy: QuestionStrategy<TranslationData> =
    createQuestionStrategy<TranslationData>({
        keyPerLine: 0,
        scorePerQuestion: 0,
            ...
            items: [
                {
                    chinese: '今年除夕你打算做什么？',
                    keyword: 'plan',
                    references: ["What do you plan to do on New Year's Eve this year?"],
                    score: 3,
                },
                {
                    chinese: '下届运动会届时将有更多的学生参加比赛。',
                    keyword: 'take part in',
                    references: [
                        'More students will take part in the competition at the next sports meeting.',
                    ],
                    score: 4,
                },
                {
                    chinese: '我们应该意识到健康对每个人来说才是最重要的。',
                    keyword: 'aware',
                    references: [
                        'We should be aware that health is the most important thing for everyone.',
                    ],
                    score: 4,
                },
                {
                    chinese: '尽管这个作业的难度超出了他的预期，他还是设法按时完成了。',
                    keyword: 'beyond',
                    references: [
                        'Although the difficulty of this assignment was beyond his expectation, he managed to finish it on time.',
                    ],
                    score: 5,
                },
            ],
...
const writingStrategy: QuestionStrategy<WritingData> = createQuestionStrategy<WritingData>({
    ...
    getDefaultValue: () => ({
        ...
        type: 'writing',
        guidance:
            '<p>你校英语组打算开展一个名为"经典名著整本阅读"的项目。你作为本校的学生李华，受委托去了解同学们对该项目的想法。</p><p>请给学校英语组写一封邮件，内容包括：</p><ol><li>描述同学们目前存在的顾虑或困难。</li><li>针对上述问题，提出具体的建议并说明理由。</li></ol>',
    }),
})
