export const highlightSubstrings = (mainString: string, substringsToMatch: string[]): string => {
    ...
    for (const substring of substringsToMatch) {
        ...
        const regex = new RegExp(
            `(?<!<mark[^>]*>)(?<!</mark[^>]*>)${substring.replace(/[/\-\\^$*+?.()|[\]{}]/g, '\\$&')}(?!</mark>)`,
            'gi',
        )
        resultString = resultString.replace(
            regex,
            match =>
                `<mark class="bg-pink-100 text-pink-900 dark:bg-secondary-100 dark:text-secondary-900 px-0.5 rounded">${match}</mark>`,
        )
    }
...
export function matchColor(
    pattern: [string | null | undefined, Color][],
    key: string,
): Color | undefined {
