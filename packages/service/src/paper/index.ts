...
import {
    ...
    setPaperPasscode,
} from '@repo/supabase/paper'
import { getUser, getUserOrThrow } from '@repo/user'
import {
    AskResponseSchema,
    QuizData,
    QuizItemsSchema,
    SectionAnswersSchema,
...
export const getPaperAction = actionClient
    ...
    .action(async ({ parsedInput: { id, passcode } }) => {
        ...
        await Kilpi.papers
            .read({ ...paper, providedPasscode: passcode })
            .authorize()
            .assert()
 ...
 */
export const getPapersByCreatorAction = actionClient.action(async () => {
    const user = await getUserOrThrow()
    await Kilpi.authed().authorize().assert()
 ...
 */
export const getPublicPapersAction = actionClient.action(async () => {
    ...
    .action(async ({ parsedInput: { id, data, baseVersion } }) => {
        ...
        const updated = await updatePaper({
            ...
            data: {
                ...data,
                tags: data.content
                    ? data.content.map(item => SECTION_NAME_MAP[item.type])
                    : undefined,
            },
        })
...
export const getPaperSubmissionAction = actionClient
    ...
    .action(async ({ parsedInput: { paperId } }) => {
        ...
        if (decision.granted) {
            const submission = await getPaperSubmission({
                paperId,
...
export const submitPaperAction = actionClient
    ...
    .action(async ({ parsedInput: { paperId, answers, score, perfectScore, passcode } }) => {
        ...
        await Kilpi.papers
            .submit({ ...paper, providedPasscode: passcode })
            .authorize()
            .assert()
...
export const fetchLeaderboardAction = actionClient
    ...
    .action(async ({ parsedInput: { paperId, passcode } }) => {
        ...
        await Kilpi.papers
            .readSubmissions({ ...paper, providedPasscode: passcode })
            .authorize()
            .assert()
...
export async function streamExplanationAction({
    quizData,
    questionNo,
    userAnswer,
}: StreamExplanationParams) {
