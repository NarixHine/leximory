...
export default function BlogHome() {
    ...
    return <Center>
        <div className='flex flex-col items-center justify-center sm:flex-row my-3'>
            <div className='border-l-2 border-l-danger/50 px-3 py-1'>
                <H disableCenter className={'text-danger text-4xl pb-2 font-semibold'} usePlayfair>
                    <span className='opacity-30'>The</span> <span className='mx-1'>Leximory</span> <span className='opacity-30'>Blog</span>
                </H>
                <H disableCenter className={'text-danger/80 text-lg leading-tight'}>
                    The tech and mind behind Leximory.
                </H>
            </div>
            <Spacer y={5}></Spacer>
            <div className='space-y-3 flex flex-wrap justify-center'>
                {posts.map((post, i) => <span key={i} className='border-danger/10 px-3 not-prose inline-block' style={{
