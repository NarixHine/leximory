...
export async function generateSingleComment(prompt: string, lib: string) {
    ...
    (async () => {
        const { textStream } = await streamText({
            ...
            ${instruction[lang]}
            `,
            prompt: `由于下文中只有一个加双重中括号的单词、词组搭配或语块，你只需要依次输出它的原文形式、原形、释义${lang === 'en' ? '、语源、同源词' : ''}即可，形如 word||original${lang === 'en' ? '||etymology||cognates' : ''}。具体形式必须保持与示例注解一致。不要附带原文和上下文。\n${lang === 'en' ? '例如如果双重中括号内是“break it down”，则对短语“break down”进行注解，而不是“break”或“break it down”。你必须在注解中附带语源，禁止省略。\n' : ''}\n\n${prompt}`,
            maxTokens: 1000
        })
...
const instruction: {
  ...
  `,
    ...
    en: `你将看到一段英文文本，你必须为一个英文学习者尽可能多地挑选其中的词汇和词组搭配添加以下注解：原文形式（必加），原形（必加），原形的字典式的简短释义（必加；含英式发音音标，音标用单斜杠包裹，不要在末尾多加一个斜杠；术语标注领域名称，例如 \`Linguistics\`，\`Computing\`，\`Medicine\`），对记忆有助益的语源（选加，同语根词加上时必加），同语根词（选加，不要选取同一词的不同词性变化，选择都含有同源部分的不同词）。尽量详尽地挑选并注解；多多注解外刊中的有价值单词、对英语学习实用的语汇；禁止反复注解同一词汇；禁止注解初级词汇。输出格式为 {{原文形式||原形||释义||(语源解释) 各个语素||语根1 (原意) → 同语根词1, 同语根词2; 语根2 (原意) → 同语根词1; …}}。词性、词义和词源注解的对象必须为词形变化的原形，例如如果原文为implications，原形为implication，则应按照implication进行注解，词性为名词，语源中不要加入单复数变化词尾-s；例如如果原文为dazzling，原形为dazzling，则应按照dazzling进行注解，词性为形容词；例如如果原文为forgiven，原形为forgive，则应按照forgive进行注解，词性为动词，语源中不要添加时态变化词尾-en。
  
  除此以外，不要修改文本其他部分的Markdown语法。直接输出注解后的文章。不要删除紧跟的标点。注解的同源词板块中，相同的语素才是应当加粗的部分（如对于ensue中的sue应加粗pursue中的sue，即“***sue*** (follow) → pur**sue** (追求)”）。注意：对于注解的同源词板块，拼写相同的词素未必同源（例如sur**veill**ance的同源词不是veil（面纱），而是vigilant（watchful），因此语源为“意即从上观察: ***sur*** (above) + ***veil*** (watch) + ***-ance*** (forming n., 表行为)”，同源词为“***veil*** → **vigil**ance (警觉)”），只要源于同一印欧语根就是同源词（例如break和fragile）。不要把词尾的非词缀字母强行放入语源。
  ...
  `,
    ...
    ja: `你将看到一段日文，你要为一个日文学习者**尽可能多地**挑选其中对学习日语有价值的和语（不注解格助词）、汉字、词组和专有名词添加以下注解：原文形式，原形（不要调换汉字），字典式释义（含假名注音、词性（名／動／形動／形／副／……）、中文和日文解释）。注意在注解词语时完整地划全，例如必须将“儚かった”中的“儚っか”视为一个词，原文形式为“儚っか”，原形为“儚い”；把“気付いて”中的“気付い”视为一个整体，原文形式为“気付い”，原型为“気付く”。注意对日文学习的实用性，不要重复注解同一词语。
