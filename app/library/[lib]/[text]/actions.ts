...
export async function generateSingleComment(prompt: string, lib: string) {
    ...
    (async () => {
        const { textStream } = await streamText({
            ...
            ${instruction[lang]}
            `,
            prompt: `由于下文中只有一个加双重中括号的语块，你只需要依次输出它的原文形式、原形、语境义${lang === 'en' ? '、语源、同源词' : ''}即可，形如 word||original${lang === 'en' ? '||etymology||cognates' : ''}。具体形式必须保持与示例注解一致。必须注解包裹的完整语块，不得截断，也不要附带原文和前后句。\n${lang === 'en' ? '例如如果双重中括号内是“break it down”，则对短语“break down”进行注解，而不是“break”或“break it down”。你必须在注解中附带语源，禁止省略。\n' : ''}\n\n${prompt}`,
            maxTokens: 1000
        })
...
const instruction: {
  ...
  `,
    ...
    ja: `你将看到一段日文，你要为一个日文学习者**尽可能多地**挑选其中对学习日语有价值的和语（不注解格助词）、汉字、词组和专有名词添加以下注解：原文形式，原形（不要调换汉字），语境下释义（汉字则标明注音、假名则标注汉字、词性（名／動／形動／形／副／……）、中文和日文解释），语源（可选）。注意在注解词语时完整地划全，例如必须将“儚かった”中的“儚っか”视为一个词，原文形式为“儚っか”，原形为“儚い”；把“気付いて”中的“気付い”视为一个整体，原文形式为“気付い”，原型为“気付く”。注意对日文学习的实用性，不要重复注解同一词语。
  
  输出格式为 {{原文形式||原形||释义||语源}}。直接输出注解后的文章。注意：注解必须遍布全文，你必须多加注解难词、高价值词。注解词汇在语境中的具体含义，若为比喻义、引申义则须指出。
  
  示例输入一：
  腕組をして枕元に坐っていると、仰向に寝た女が、静かな声でもう死にますと云う。女は長い髪を枕にしいて、輪郭の柔らかな瓜実顔をその中に横たえている。真白な頬の底に温かい血の色がほどよく差して、唇の色は無論赤い。とうてい死にそうには見えない。
...
>
> 鳥を可愛がるもうひとりのおじさんは、鳥の怪我を心配しジェットコースターほど不安で、鳥籠の扉を開ける事はないでしょう。いつか鳥籠の扉が開いた時、鳥はこの場所からにげだしてしまいたくなります。逃げてしまった鳥に対し、おじさんは怒り狂うでしょう。

  示例输出二：
> おじさんは、鳥籠を{{可哀想||可哀想||**［形動］（かわいそう／可怜）**見ていて不憫である。気の毒である。}}に思い、鳥籠の扉を開けてあげるでしょう。鳥が{{こわがる||こわがる||**［動］（怖がる／害怕）**恐ろしさを感じる。}}なら飛び立たなくてもいい。飛び立って自由に羽ばたき、そのまま帰ってこなくてもいい。鳥はそんな人の元に帰ってくる事があるかもしれませんが、おじさんは鳥が帰ってくる事を{{強制||強制||**［名・形動］（きょうせい／强制）**無理にさせること。}}したりしません。鳥が自由に生き生きと生きる事を、おじさんは心から喜びます。  
>   
> 鳥を可愛がるもうひとりのおじさんは、鳥の怪我を心配し{{ジェットコースター||ジェットコースター||**［名］（ジェットコースター／过山车）**遊園地の乗り物の一種。ここではごく不安定の状態を指す。||和製英語（roller coasterから）：jet（喷气机）＋coaster（汽船）}}ほど不安で、鳥籠の扉を開ける事はないでしょう。いつか鳥籠の扉が開いた時、鳥はこの場所から{{にげだし||にげだす||**［動］（逃げ出す／逃走）**逃げていく。}}てしまいたくなります。逃げてしまった鳥に対し、おじさんは{{怒り狂う||怒り狂う||**［動］（おこりくるう／暴跳如雷）**ひどく怒ること。}}でしょう。  
  `,
}
