...
export async function generateSingleComment(prompt: string, lib: string) {
    ...
    (async () => {
        const { textStream } = await streamText({
            ...
            ${instruction[lang]}
            `,
            prompt: `下文中只有一个加双重中括号的语块，你仅需要依次输出它的原文形式、原形、语境义（含例句）${lang === 'en' ? '、语源、同源词' : ''}${lang === 'ja' ? '、语源（可选）' : ''}即可，但***必须在语境义部分附上该词的例句（不要在释义和例句间加双竖线）***。形式如${lang === 'en' ? 'word||original||meaning here: *an example sentence*||etymology||cognates。例如：transpires||transpire||**v. 被表明是** \`trænˈspaɪə\` happen; become known\n\n> It later transpired that he was a spy.||原形容水汽“升腾”: ***trans-*** (across) + ***spire*** (breathe) ||***trans-*** (across) → **trans**fer (转移), **trans**late (翻译); ***spire*** (breathe) → in**spire** (吹入灵感, 鼓舞)。' : ''}${lang === 'ja' ? '単語||原形||意味：*例文*||語源。例如：可哀想||可哀想||**［形動］（かわいそう／可怜）**気の毒である。\n\n> 彼女は可哀想に見えた。||**「可哀想」は当て字**：**かわい**（可悲）＋**そう**（……的样子）' : ''}${lang === 'zh' ? '词语||词语||释义：*例文*' : ''}。必须注解包裹的完整语块，不得截断，也不要附带原文和前后句。\n${lang === 'en' ? '例如如果双重中括号内是“break it down”，则对短语“break down”进行注解，而不是“break”或“break it down”。你必须在注解中附带语源，禁止省略。\n' : ''}\n\n${prompt}`,
            maxTokens: 1000
        })
