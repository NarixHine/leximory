...
export default function EditableH() {
    ...
    return isEditing ? <div className='flex justify-center'><Input value={title} onValueChange={setTitle} style={{ fontFamily: `${ENGLISH_SERIF.style.fontFamily}, ${CHINESE.style.fontFamily}` }} size='lg' classNames={{ input: 'text-center text-3xl' }} ></Input></div> : <H usePlayfair={lang === 'zh' || lang === 'en'} useNoto={lang === 'ja'} className={'sm:text-4xl mb-2 text-3xl'}>{title}</H>
}
