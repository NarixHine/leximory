        ...
        <CardBody>
            <div className='mt-2'>
                {url ? <audio
                    ...
                    className='w-full'
                    src={url}
                /> : <Button data-umami-event={status === 'ungenerated' ? '音频生成' : '音频播放'} isLoading={status === 'loading' || status === 'generating'} isDisabled={status === 'lengthy'} variant='flat' radius='full' color='danger' startContent={<PiPlayCircleDuotone />} size='sm' onPress={() => action()}>
                    {
