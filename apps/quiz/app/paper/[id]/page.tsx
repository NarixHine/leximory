...
async function Content({
    ...
    params: PaperPageProps['params']
    searchParams: PaperPageProps['searchParams']
}) {
    const [{ id }, { passcode }] = await Promise.all([params, searchParams])
    const { content, answers, title, feedback, score } = await getData({
        id: parseInt(id),
        passcode,
    })
    const questionCount = content.reduce(
        (count, item) =>
            count + applyStrategy(item, (strategy, data) => strategy.getQuestionCount(data)),
        0,
    )
