...
import {
    buildSummaryMarkingPrompt,
    buildTranslationMarkingPrompt,
    buildWritingScoringPrompt,
    buildWritingAnalysisPrompt,
} from '@repo/service/ai/prompts/subjective'
...
export const markSubjectiveSections = inngest.createFunction(
    {
        id: 'mark-subjective-sections',
        retries: 2,
        idempotency: 'event.data.submissionId',
        triggers: [submissionMarking],
    },
    async ({ event, step }) => {
        ...
        const subjectiveSections = paper.content.filter(section =>
        ...
        for (const section of subjectiveSections) {
            if (section.type === 'summary') {
                ...
                feedback[section.id] = await step.run(`mark-summary-${section.id}`, async () => {
                    ...
                    const { object } = await generateObject({
                        ...FLASH_AI,
                        prompt,
                        schema: SummaryFeedbackSchema.omit({
                            type: true,
                            copiedChunks: true,
                            contentScore: true,
                            totalScore: true,
                    ...
                    const essentialFulfilled = object.essentialItemResults.filter(
                        r => r.fulfilled,
                    ).length
            ...
            if (section.type === 'translation') {
                ...
                const hasAnyAnswer = Object.values(sectionAnswers).some(
                    v => v != null && v.trim() !== '',
                )
                ...
                feedback[section.id] = await step.run(
                    `mark-translation-${section.id}`,
                    async () => {
        ...
        const subjectiveScore = Object.values(feedback).reduce((sum, fb) => sum + fb.totalScore, 0)
