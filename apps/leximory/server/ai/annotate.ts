...
export const articleAnnotationPrompt = async (
    lang: Lang,
    input: string,
    ...
    system: `
        ...
        ${!isFirstChunk ? '注意：你正在处理的是文章的后续部分（而非全文开头），所以无需对最开始的单词使用Small caps语法处理。' : ''}
    ...
    `,
    prompt: `
    ${lang !== 'en' && lang !== 'fr' ? '' : '你要为外语学习者注解一切高阶或罕见词汇，必须添加语源。'}注解必须均匀地遍布下文。
...
export const annotateParagraph = async ({
    content,
    lang,
    userId,
    autoTrim = true,
