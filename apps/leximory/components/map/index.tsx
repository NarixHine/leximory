/**
 * Vendored from mapcn (https://mapcn.dev) — the MapLibre-powered map registry.
 * Adapted to Leximory: HeroUI design tokens and react-icons in place of
 * shadcn/ui tokens and lucide-react. Upstream: `npx shadcn@latest add @mapcn/map`.
 */
'use client'
...
import { PiX, PiMinus, PiPlus, PiCrosshairSimple, PiCornersOut, PiSpinnerGap } from 'react-icons/pi'
    ...
    return (
        <div className='absolute inset-0 z-10 flex items-center justify-center'>
            <div className='flex gap-1'>
                <span className='bg-default-400/60 size-1.5 animate-pulse rounded-full' />
                <span className='bg-default-400/60 size-1.5 animate-pulse rounded-full [animation-delay:150ms]' />
                <span className='bg-default-400/60 size-1.5 animate-pulse rounded-full [animation-delay:300ms]' />
            </div>
        </div>
    )
}
...
function DefaultMarkerIcon() {
    return (
        <div className='relative h-4 w-4 rounded-full border-2 border-white bg-blue-500' />
    )
}
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
    return (
        <button
            ...
            className='focus-visible:ring-primary hover:bg-default-100 text-default-700 absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset'
        >
...
function MarkerPopup({
    ...
    return createPortal(
        <div
            className={cn(
                'bg-content1 text-default-700 relative max-w-62 rounded-md border p-3',
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
    ...
    return createPortal(
        <div
            className={cn(
                'bg-default-600 text-default-50 pointer-events-none rounded-md px-2 py-1 text-xs text-balance',
                'animate-in fade-in-0 zoom-in-95 duration-200 ease-out',
                className,
            )}
        >
    ...
    return (
        <div
            className={cn(
                ...
                'text-default-700 text-[10px] font-medium',
...
function ControlGroup({ children }: { children: React.ReactNode }) {
    return (
        <div className='border-default-200/60 bg-content1/40 [&>button:not(:last-child)]:border-default-200/60 flex flex-col overflow-hidden rounded-lg border backdrop-blur-sm [&>button:not(:last-child)]:border-b'>
            {children}
        </div>
    )
}
    ...
    return (
        <button
            ...
            className={cn(
                ...
                'hover:bg-default-100 dark:hover:bg-default-100/40',
                ...
                <ControlGroup>
                    ...
                    >
                        {waitingForLocation ? (
                            ...
                            <PiCrosshairSimple className='size-4' />
                        )}
                    </ControlButton>
                </ControlGroup>
            )}
    ...
    return (
        <ControlButton onClick={onClick} label='Reset bearing to north'>
            ...
            >
                ...
                <path d='M12 22L16 12H12V22Z' className='fill-default-400/60' />
                <path d='M12 22L8 12H12V22Z' className='fill-default-400/30' />
            </svg>
        </ControlButton>
    )
}
...
function MapRoute({
    ...
    useEffect(() => {
        ...
        const raise = () => {
            ...
            const owned = [layerId, ...childLayersRef.current].filter(entry => map.getLayer(entry))
 ...
 */
function RouteProgress({ color, width, opacity, dashArray }: RouteProgressProps) {
    const { map, isLoaded } = useMap()
