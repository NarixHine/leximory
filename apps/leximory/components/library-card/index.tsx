    ...
    return (
        <div className='block break-inside-avoid rounded-[2.875rem] bg-default-50 p-3.5'>
            <Card
                ...
                shadow='none'
                href={`/library/${id}`}
                className='p-0 bg-transparent rounded-4xl'
            >
                <CardBody className='p-0 bg-default-100 rounded-4xl px-6 pb-7 pt-5'>
                    <span className='mb-2 inline-block font-semibold text-default-400'>
                        {getLanguageStrategy(lang as Lang).name}
                    </span>
                    <h2 className='font-fancy text-3xl leading-tight text-foreground text-balance'>
                        {name}
                    </h2>
                </CardBody>
            </Card>
