...
export function DiscreteProgress({
    ...
    if (onClick) {
        return (
            <button onClick={onClick} className='flex items-center gap-2 group cursor-pointer'>
                {showLang && langCode && (
                    <span className='text-sm font-mono uppercase text-default-500'>{langCode}</span>
                )}
                {bar}
                {showIcon && icon}
            </button>
