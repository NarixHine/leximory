    ...
    return (
        <section className='relative isolate overflow-hidden rounded-2xl text-default-700'>
            <div className='relative flex flex-wrap flex-row items-end space-x-4 gap-2'>
                ...
                <div className='flex gap-0 self-end' aria-label={`连续打卡 ${streak.total} 天`}>
                    {streak.checkDays.map(day => (
                        <div
                            key={day.date}
                            className={cn('grid size-8 place-items-center rounded-full')}
                        >
                            {day.completed ? (
            ...
            <div className='relative flex flex-wrap flex-row items-end space-x-4 gap-2'>
                <div className='min-w-0'>
                    <p className='font-kaiti text-xl tracking-normal text-secondary-300/70'>
                        连续打卡{' '}
                        <span className='inline-block h-7.25 w-10 translate-y-1 rounded-md bg-default-200/80 align-baseline animate-pulse' />{' '}
                        天
                    </p>
                </div>

                <div className='flex gap-0 self-end' aria-label='正在加载连续打卡' aria-busy='true'>
                    {Array.from({ length: 7 }, (_, index) => (
                        <div key={index} className='grid size-8 place-items-center rounded-full'>
