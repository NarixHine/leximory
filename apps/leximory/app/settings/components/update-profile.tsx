...
import { MAX_AVATAR_SIZE, MAX_FILE_SIZE, SIGN_IN_URL } from '@repo/env/config'
...
export function SectionCard({ children, footer, title, onSubmit }: { children: React.ReactNode, footer?: React.ReactNode, title: string, onSubmit?: (e: React.FormEvent<HTMLFormElement>) => void }) {
    return (<form onSubmit={onSubmit} className='w-full h-full'>
        <Card shadow='none' className='bg-default-50/50 p-0 rounded-4xl w-full h-full'>
            ...
            <CardBody className='px-6 pt-2 pb-5'>{children}</CardBody>
            {footer && <CardFooter className='px-6 pt-2 pb-5'>{footer}</CardFooter>}
        </Card>
    </form>
    )
}
    ...
    return (
        <SectionCard title='用户名' footer={<Button type='submit' className='w-full h-10' variant='flat' color='default' isLoading={isLoading} startContent={<PiUser size={20} />}>
            ...
            <div className='space-y-2'>
                <label htmlFor='username' className='font-medium text-sm leading-none'>用户名</label>
...
export function AvatarSection({ image }: { image?: string }) {
    ...
    const handleUpload = async () => {
        ...
        if (selectedFile.size > MAX_AVATAR_SIZE) {
            toast.error(`请上传小于 ${MAX_AVATAR_SIZE / 1024 / 1024}MB 的图片`)
            return
        }
        setIsLoading(true)
        try {
    ...
    return (
        <SectionCard title='头像'>
            <div className="flex flex-col items-center gap-4">
                <div className='flex justify-center items-center bg-default-100 border border-default-200 rounded-full w-20 h-20 overflow-hidden'>
                    {preview ? (
...
function EmailSection({ onSuccess }: { onSuccess?: () => void }) {
    ...
    return (
        <SectionCard title='邮箱' footer={<Button type='submit' className='w-full h-10' variant='flat' color='default' isLoading={isLoading} startContent={<PiEnvelopeSimple size={20} />}>
            ...
            <div className='space-y-2'>
                <label htmlFor='email' className='font-medium text-sm leading-none'>电子邮箱地址</label>
...
function PasswordSection({ onSuccess }: { onSuccess?: () => void }) {
    ...
    return (
        <SectionCard title='密码' onSubmit={async e => {
            ...
            <div>
                <label htmlFor='password' className='font-medium text-sm leading-none'>新密码</label>
                <Input id='password' type='password' value={password} onChange={e => setPassword(e.target.value)} placeholder='新密码' className='h-10' />
            </div>
            <div className='my-4'>
                <label htmlFor='confirm-password' className='font-medium text-sm leading-none'>确认密码</label>
...
export default function UpdateProfile({ userId, username, image, className, ...props }: React.ComponentPropsWithoutRef<'div'> & { userId: string, username?: string, image?: string }) {
    return (
        <div className={cn('justify-center items-center gap-3 grid grid-cols-1 md:grid-cols-2 bg-background w-full min-h-[60vh]', className)} {...props}>
    ...
    return (
        <div className={cn('justify-center items-center gap-3 grid grid-cols-1 md:grid-cols-2 bg-background w-full min-h-[60vh]', className)} {...props}>
            <SectionCard title='用户名'>
                <div className='bg-default-100 rounded-xl w-full h-10 animate-pulse' />
            </SectionCard>
            <SectionCard title='头像'>
                <div className='bg-default-100 rounded-xl w-full h-10 animate-pulse' />
            </SectionCard>
