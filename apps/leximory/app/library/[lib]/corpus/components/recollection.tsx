...
export default function Recollection({
    ...
    const [recol, loadRecol, isLoading] = useActionState(
        async (recol: { words: typeof words; cursor: string; more: boolean }) => {
        ...
        <div>
            ...
            <div className='grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-3 w-full'>
                {recol.words.map(({ word, id, date }, index, array) => {
                    const anotherDay =
                        array[index + 1] && !moment(date).isSame(array[index + 1].date, 'day')
                    return (
                        <Fragment key={id}>
                            <div className='w-full min-h-20 h-full flex flex-col justify-center items-center rounded-4xl bg-default-50 p-3 transition-colors hover:bg-default-100/50'>
                                <Markdown
                                    md={word}
                                    disableSave={welcomes.includes(word)}
                                    deleteId={
                                        isReadOnly || welcomes.includes(word) ? undefined : id
                                    }
                                />
                            </div>
                            {anotherDay && (
                                <div className='w-full py-3 flex flex-col px-4 col-span-full'>
                                    <span className='text-xs font-semibold tracking-widest text-default-400'>
                                        {momentSH(array[index + 1].date)
                                            .startOf('day')
                                            .format('ll')}
                                    </span>
                                </div>
            ...
            {recol.more && (
                <form action={loadRecol} className='flex justify-center mt-8'>
                    <Button
                        type='submit'
                        radius='full'
                        variant='flat'
