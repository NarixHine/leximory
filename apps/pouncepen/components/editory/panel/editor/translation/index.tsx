...
export default function TranslationEditor({
    ...
    return (
        <div className='flex flex-col gap-4 before:content-["Translation"] before:text-secondary-300 before:font-bold before:-mb-2 my-5'>
            {data.items.map((item, index) => (
                <div
                    key={index}
                    className='flex flex-col gap-2 border border-default-200 rounded-medium p-3'
                >
                    ...
                    <List
                        ...
                        remove={ref =>
                            updateItem(index, {
                                references: item.references.filter(r => r !== ref),
                            })
                        }
                    />
                </div>
