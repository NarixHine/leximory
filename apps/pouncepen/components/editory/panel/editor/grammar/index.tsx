...
export default function GrammarEditor({
    ...
    return (
        <div className='flex flex-col gap-2 before:content-["Grammar"] before:text-secondary-300 before:font-bold before:-mb-4 my-5'>
            <Modal isOpen={isOpen} onOpenChange={onOpenChange} isKeyboardDismissDisabled>
                <ModalContent>
                    {onClose => (
                        <Form
                            className='w-full'
                            onSubmit={e => {
                                ...
                                newData.hints = Object.fromEntries(
                                    Object.entries(newData.hints).filter(
                                        ([key, value]) => blanks.includes(key) && value,
                                    ),
                                )
