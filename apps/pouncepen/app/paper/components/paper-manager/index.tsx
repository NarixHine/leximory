...
export function PaperManagerHeader({
    isCreating,
    handleCreate,
}: {
    isCreating?: boolean
    ...
    return (
        <div className='flex items-center my-8'>
            ...
            <Button variant='flat' onPress={() => router.push('/local-editor')} className='mr-2'>
                本地编辑器
            </Button>
            <Button
                color='primary'
...
export function PaperManager({ papers: initialPapers }: { papers: PaperOverview[] }) {
    ...
    const { execute: executeToggleVisibility, isPending: isTogglingVisibility } = useAction(
        togglePaperVisibilityAction,
        {
            onSuccess: result => {
    ...
    const { execute: executeGeneratePasscode, isPending: isGeneratingPasscode } = useAction(
        generatePaperPasscodeAction,
        {
            onSuccess: result => {
                setPapers(prev => prev.map(p => (p.id === result.data.id ? result.data : p)))
    ...
    const { execute: executeRevokePasscode, isPending: isRevokingPasscode } = useAction(
        revokePaperPasscodeAction,
        {
            onSuccess: result => {
                setPapers(prev => prev.map(p => (p.id === result.data.id ? result.data : p)))
    ...
    const { execute: executeRotatePasscode, isPending: isRotatingPasscode } = useAction(
        rotatePaperPasscodeAction,
        {
            onSuccess: result => {
                setPapers(prev => prev.map(p => (p.id === result.data.id ? result.data : p)))
                                            ...
                                            <div className='flex items-center'>
                                                ...
                                                {!paper.public && (
                                                    <Button
                                                        ...
                                                        size='sm'
                                                        color={
                                                            paper.passcode ? 'secondary' : 'default'
                                                        }
                                                        onPress={() => {
                                                            setSharePaper(paper)
                                                            shareModal.onOpen()
                        ...
                        (sharePaper.passcode ? (
                            <>
                                ...
                                <ModalFooter className='flex-col sm:flex-row gap-2'>
                                    <Button
                                        ...
                                        variant='flat'
                                        startContent={
                                            !isRevokingPasscode && <XCircleIcon size={20} />
                                        }
                                    ...
                                    <Button
                                        ...
                                        variant='flat'
                                        startContent={
                                            !isRotatingPasscode && <ArrowsClockwiseIcon size={20} />
                                        }
                            ...
                            <>
                                ...
                                <ModalBody>
                                    <p>
                                        为此试卷生成一个<b>私有分享链接</b>
                                        。任何持有链接者可以直接访问、作答并查看排行榜。
                                    </p>
                                    <p>
                                        试卷本身依旧<b>不会出现</b>在公开列表中，且你可以
                                        <b>随时关闭</b>分享或重新生成链接使旧链接失效。
                                    </p>
                                </ModalBody>
                                <ModalFooter>
                                    ...
                                    <Button
                                        color='primary'
                                        startContent={
                                            !isGeneratingPasscode && <LinkIcon size={20} />
                                        }
                                        onPress={() =>
                                            executeGeneratePasscode({ id: sharePaper.id })
                                        }
